    ...
    private SelectDSLCompleter buildCompleter(Book req) {
        SelectDSLCompleter completer = new SelectDSLCompleter() {
            ...
            public Buildable<SelectModel> apply(QueryExpressionDSL<SelectModel> selectModelQueryExpressionDSL) {
                ...
                String ji = req.getJi();
                if (StringUtils.isNotBlank(ji)) {
                    where.and(BookSql.ji, SqlBuilder.isEqualTo(req.getJi()));
                }
                String qu = req.getQu();
                if (StringUtils.isNotBlank(qu)) {
                    where.and(BookSql.qu, SqlBuilder.isEqualTo(req.getQu()));
                }
                String ma = req.getMa();
                if (StringUtils.isNotBlank(ma)) {
                    where.and(BookSql.ma, SqlBuilder.isEqualTo(req.getMa()));
                }
                return where;
            }
        };
