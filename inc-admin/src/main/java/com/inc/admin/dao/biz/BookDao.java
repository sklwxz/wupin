...
public interface BookDao {
    BasicColumn[] selectList = BasicColumn.columnList(id, name, status, user, ji,qu,ma);
    ...
    @Results(id="BookResult", value = {
        ...
        @Result(column="ji", property="ji", jdbcType=JdbcType.VARCHAR),
        @Result(column="qu", property="qu", jdbcType=JdbcType.VARCHAR),
        @Result(column="ma", property="ma", jdbcType=JdbcType.VARCHAR)
    })
    ...
    default int insert(Book record) {
        return MyBatis3Utils.insert(this::insert, record, book, c ->
            ...
            .map(ji).toProperty("ji")
            .map(qu).toProperty("qu")
            .map(ma).toProperty("ma")
        );
    }
    ...
    default int insertSelective(Book record) {
        return MyBatis3Utils.insert(this::insert, record, book, c ->
            ...
            .map(ji).toPropertyWhenPresent("ji", record::getJi)
            .map(qu).toPropertyWhenPresent("qu", record::getQu)
            .map(ma).toPropertyWhenPresent("ma", record::getMa)
        );
    }
    ...
    static UpdateDSL<UpdateModel> updateAllColumns(Book record, UpdateDSL<UpdateModel> dsl) {
        ...
        .set(ji).equalTo(record::getJi)
        .set(qu).equalTo(record::getQu)
        .set(ma).equalTo(record::getMa);
    }
    ...
    static UpdateDSL<UpdateModel> updateSelectiveColumns(Book record, UpdateDSL<UpdateModel> dsl) {
        ...
        .set(ji).equalToWhenPresent(record::getJi)
        .set(qu).equalToWhenPresent(record::getQu)
        .set(ma).equalToWhenPresent(record::getMa);
    }
